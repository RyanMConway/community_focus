...
export async function getGoogleReviews(): Promise<Review[]> {
  ...
  try {
    ...
    if (!data.result || !data.result.reviews) {
      console.warn("⚠️ Google API returned OK, but no reviews found.");
      return [];
    }

    // --- NEW: FILTER LOGIC ---
    // Only keep reviews with 4 or 5 stars
    const validReviews = data.result.reviews.filter((r: any) => r.rating >= 4);

    const cleanReviews: Review[] = validReviews.map((r: any) => ({
    ...
    console.log(`✅ Fetched ${cleanReviews.length} high-quality reviews from Google.`);
    return cleanReviews;
